...
class TTSRequest(BaseModel):
    text: str
    use_rvc: bool = True  # Default to True
...
async def convert_text(request: TTSRequest):
    try:
        ...
        duration_sec = 1
        sample_rate = 24000
        silent_audio = np.zeros(int(duration_sec * sample_rate), dtype=np.float32)
        sf.write(tts_output, silent_audio, sample_rate)

        if not request.use_rvc:
            return FileResponse(tts_output, media_type="audio/wav", filename="output.wav")
        ...
        return FileResponse(rvc_output, media_type="audio/wav", filename="output.wav")
    except HTTPException:
        raise
    except Exception as e:
...
async def download_model(request: ModelDownloadRequest):
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            has_pth = any(f.endswith('.pth') for f in os.listdir(model_dir))
            if not has_pth:
                shutil.rmtree(model_dir)
